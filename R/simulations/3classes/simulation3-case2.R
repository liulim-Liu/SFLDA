...
simulation3.case2 <- function(){
  ...
  for (k in 1:1){
    ...
    for (j in 1:100){
      result0 <- vector(mode='list', length=600)
      result1 <- vector(mode='list', length=600)
      ...
      for (i in 1:600){     ## i from 1 to 200
        ...
        ep <- rnorm(40, 5, sigma.c)  ## larger between time point noise: rnorm(40)

        y_ij.t.1 = delta1 + eta1$eta0 + eta1$eta1 * t + eta1$eta2 * t^2 + eta1$eta3 * t^3 + eta1$eta4 * t^4 + eta1$eta5 * sin(eta1$eta6 * t) + ep
    ...
    ## replace time 5:15 from simulation 0 to simulation 1
    df0[df0$time %in% c(5:15),] <- df1[df1$time %in% c(5:15),]

    #write.csv(df0, paste("simulation_n300_p100.10_t40_c3_tp2.csv", sep = ""), row.names=FALSE)

  }


  #data <- read.csv("simulation_n300_p100.10_t40_c3_tp2.csv")
  #View(data)
  data <- df0
  ...
  #write.csv(train.df, "train_simulation_n300_p100.10_t40_c3_tp2.csv", row.names=FALSE)
  #write.csv(test.df, "test_simulation_n300_p100.10_t40_c3_tp2.csv", row.names=FALSE)
