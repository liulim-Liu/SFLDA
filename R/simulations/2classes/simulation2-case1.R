...
simulation2.case1 <- function(){
  ...
  for (k in 1:1){
    ...
    for (j in 1:100){
      result <- vector(mode='list', length=400)
      eta <- find_eta()
      ...
      for (i in 1:400){     ## i from 1 to 200
        ...
        if (i > 200 & j <= 10) {
          ...
          ## same curve as the previous or nor
          if (i == 201){
            if (runif(1,0,1) >= 0.5){
              delta.c <- delta.c/100
              eta <- find_eta()
    ...
    #write.csv(df, paste("simulation_n200_p100.10_t40_c2_small.csv", sep = ""), row.names=FALSE)
  }

  #data <- read.csv("simulation_n200_p100.10_t40_c2_small.csv")
  #View(data)
  data = df
