...
simulation2.case2 <- function(){
  ...
  for (k in 1:1){
    ...
    for (j in 1:100){
      result0 <- vector(mode='list', length=400)
      result1 <- vector(mode='list', length=400)
    ...
    #[question here]: the change is a sudden change, not a smooth change
    ...
    #write.csv(df0, paste("simulation_n200_p100.10_t40_c2_tp2.csv", sep = ""), row.names=FALSE)

  }
  #data <- read.csv("simulation_n200_p100.10_t40_c2_tp2.csv")
  #View(data)
  data <- df0
